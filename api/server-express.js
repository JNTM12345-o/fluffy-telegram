...
function clearRoomTimers(room) {
  ...
  if (room.feedbackTimer) { clearTimeout(room.feedbackTimer); room.feedbackTimer = null; }
}
...
function startRacePhase(room) {
  ...
  room.raceTimer = setTimeout(() => {
    ...
    room.state.roundStartTime = Date.now();
    room.io.to(room.state.roomCode).emit('race_won', { playerId: randomPlayer.id });
    broadcastState(room);
...
function broadcastState(room) {
  room.io.to(room.state.roomCode).emit('room_state', { state: room.state });
}

function handleLeave(socket) {
  const info = clientRooms.get(socket.id);
  if (!info) return;
...
io.on('connection', (socket) => {
  ...
  socket.on('message', (msg) => {
    try {
      ...
      switch (data.type) {
        case 'create_room': {
          if (clientRooms.has(socket.id)) {
            socket.emit('message', { type: 'error', message: '请先离开当前房间' });
            return;
          }
          ...
          const player = {
            id: playerId,
            nickname: (data.nickname || '玩家1').substring(0, 6),
            score: 0,
            isHost: true,
            isReady: true,
            isConnected: true,
          ...
          const room = {
            state: {
              ...
              phase: 'waiting',
              currentRound: -1,
              config: { ...data.config, totalRounds: 10 },
          ...
          clientRooms.set(socket.id, { roomCode, playerId });
          socket.join(roomCode);
          socket.emit('message', { type: 'joined', playerId, state: room.state });
          break;
        }
        ...
        case 'join_room': {
          ...
          if (!room) {
            socket.emit('message', { type: 'error', message: '房间不存在' });
            return;
          }
          if (room.state.phase !== 'waiting') {
            socket.emit('message', { type: 'error', message: '游戏已开始，无法加入' });
            return;
          }
          if (room.state.players.length >= room.state.config.maxPlayers) {
            socket.emit('message', { type: 'error', message: `房间已满（限${room.state.config.maxPlayers}人）` });
            return;
          }
          if (room.state.players.some((p) => p.nickname === data.nickname)) {
            socket.emit('message', { type: 'error', message: '昵称已被使用' });
            return;
          }
          const playerId = generatePlayerId();
          const player = {
          ...
          clientRooms.set(socket.id, { roomCode: room.state.roomCode, playerId });
          socket.join(room.state.roomCode);
          socket.emit('message', { type: 'joined', playerId, state: room.state });
          io.to(room.state.roomCode).emit('message', { type: 'room_state', state: room.state });
          break;
        }
        ...
        case 'start_game': {
          ...
          const player = room.state.players.find((p) => p.id === info.playerId);
          if (!player?.isHost) {
            socket.emit('message', { type: 'error', message: '只有房主可以开始游戏' });
            return;
          }
          const connectedPlayers = room.state.players.filter(p => p.isConnected);
          if (connectedPlayers.length < 2) {
            socket.emit('message', { type: 'error', message: '至少需要 2 名玩家' });
            return;
          }
        ...
        case 'race_answer': {
          ...
          room.state.roundPhase = 'answer';
          room.state.roundStartTime = Date.now();
          io.to(room.state.roomCode).emit('race_won', { playerId: info.playerId });
          broadcastState(room);
        ...
        case 'submit_answer': {
          ...
          handleSubmitAnswer(room, info.playerId, data.selectedIndex);
          break;
        }
      ...
      socket.emit('message', { type: 'error', message: '无效的消息格式' });
    }
  });

  socket.on('disconnect', () => {
    handleLeave(socket);
    console.log(`[-] 断开 | 当前房间数: ${rooms.size}`);
  });
});
